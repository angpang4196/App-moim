...
public class MoimJoinTaskController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		SqlSession sqlSession = factory.openSession(true); // openSession(false) >>> autoCommit x

		String target = req.getParameter("target"); // 모임 ID
		// 모임 ID 에 해당하는 정보를 읽어온 후 (myBatis 처리)
		...
		User one = (User)req.getSession().getAttribute("logonUser");
		// 참석정보를 등록시켜도 된다면
		Map<String, Object> params = new HashMap<>();
		params.put("moimId", target);
		params.put("userId", one.getId());
		params.put("status", found.getType().equals("public") ? 2 : 1);
		
		int r = sqlSession.insert("attendances.create", params);
		// 이 모임 정보의 타입을 확인해서 private 이면 status를 1로 등록 / 아니면 2로 등록 (myBatis 처리)

		// public 인 경우에는 이 해당모임의 current 값을 증가 (myBatis 처리)
